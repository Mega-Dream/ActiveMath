...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	//［問題関連の宣言と初期化］　
	//●問題のロード
	struct Kadai Mondai = { 0 }; //Mondaiを入れる
	int SyoumonKosuu;
	LoadMondai(".\\File\\Mondai\\Mondai01.ack", &Mondai, &SyoumonKosuu, 1); //ディレクトリを取得する（０以外）
	...
	LoadMondaiSet(LocalDir, MsgBoxForm, MsgBoxFormKosuu, MsgBox, MsgBoxKosuu,
		Monster, MonsterMax, &MonsterKosuu, NULL, ColumnTitle, &ColumnKosuu, 1, 1);//発行済み最大ID（NULL）//フォント・タグをロードする（０以外）//ジョイパッドの割り当てをロードする（０以外）
	...
	while (!ScreenFlip() && !ProcessMessage() && !ClearDrawScreen() && !MathgpUpdateJoypad()) {
		...
		for (int k = 1; k < ColumnKosuu; k++) DrawFormatString(700, 80 + 30 * k, TextColor, "%s:%d", ColumnTitle[k], Monster[MonsterNum].Column[k]);//モンスターのカラム（モンスターID（0番）を除く）
