...
int Battle(struct PLAYER *Player) {
	...
	//●ステータスエリア
	struct AREACONTROL StatusArea = { 0 };
	int AreaKosuu;
	LoadAreaSet(".\\File\\StatusArea.csv", &StatusArea, &AreaKosuu, 1, NULL);
	StatusArea.ParentArea_p = &Entire;
	//［パッド関連の宣言と初期化］　
	//●パッドのロード
	//（パッドマネージャー）
	...
	//●問題関連ファイルのロード（メッセージフォーム，メッセージボックス，モンスター，フォントタグ設定（フォント，フォント画像，タグ設定））
	//（問題）
	...
	int MsgBoxFormKosuu_Confirm = 1;
	static struct MsgBoxForm MsgBoxForm_Confirm;
	int MsgBoxKosuu_Confirm = 1;
	...
	int MsgBoxFormKosuu_Judge = 1;
	static struct MsgBoxForm MsgBoxForm_Judge;
	int MsgBoxKosuu_Judge = 1;
	...
	int DamageSound1 = LoadSoundMem(".\\File\\MsgBattle\\Sound\\button60.mp3");
	int DamageSound2 = LoadSoundMem(".\\File\\MsgBattle\\Sound\\attack01.mp3");
	//●基準線
	...
	StatusArea.Location[0] = MsgBox[0].Location[0];
	StatusArea.Location[1] = MondaiLine - StatusArea.Height;
	...
	while (!ScreenFlip() && !ProcessMessage() && !ClearDrawScreen() && !MathgpUpdateJoypad()) {
		...
		static int GaugeRate;//経過率（％）（ゲージの長さ）
		if (GaugeFlag != 0) {//ゲージ作動時
			GaugeFrame++;//ゲージを進める
			Time_Elapsed = GaugeFrame / (float)FPS;//経過時間（秒）
			GaugeRate = 100 * Time_Elapsed / Monster[MonsterNum[MondaiNum]].Column[1];//経過率（％）（ゲージの長さ）
		}
		//●ゲージ満タン
		if (GaugeRate >= 100) {
		...
		if (MondaiNum >= 0) {
			...
			//DrawLine(0, MonsterBaseLine1, WindowWidth, MonsterBaseLine1, Red, 1);//テスト用
			...
			if (MonsterImgFlag != 0) {
				...
				if (GaugeFlag != 0) {//モンスターが表示されていてもゲージが停止時はゲージを表示しない
					//（棒ゲージ）※ゲージが満タンになればモンスターの攻撃
					static int GageColor1 = GetColor(255, 255, 255), GageColor2 = GetColor(204, 204, 204), GageColor3 = GetColor(255, 0, 0);
					int GaugeX = Entire.Nest[0] + 53 + (MsgBox[0].Width - 104) / 2;//棒ゲージの始点X
					int GaugeY = MonsterY - 30;//棒ゲージの始点Y
					DrawBox(GaugeX, GaugeY, GaugeX + 104, GaugeY + 14, GageColor1, true); //棒ゲージ
					DrawBox(GaugeX + 2, GaugeY + 2, GaugeX + 102, GaugeY + 12, GageColor2, true); //棒ゲージ
					DrawBox(GaugeX + 2, GaugeY + 2, GaugeX + 2 + GaugeRate, GaugeY + 12, GageColor3, true); //棒ゲージ
				}
			}
			//DrawLine(0, MonsterBaseLine2, WindowWidth, MonsterBaseLine2, Red, 1);//テスト用
			//DrawLine(0, MondaiLine, WindowWidth, MondaiLine, Red, 1);//テスト用
			//●開始メッセージ
			if (Scene == -1) {
				static int Count = 0;
				Count++;
		...
		//●ステータス
		ShowArea(&StatusArea, 1);
		static int NameWidth = GetDrawStringWidth(Player->Name, strlen(Player->Name));
		static int NameX = (StatusArea.Nest[2] - StatusArea.Nest[0] - NameWidth) / 2;
		static int Status_TextColor = GetColor(255, 255, 255);
		DrawFormatString(StatusArea.Nest[0] + NameX, StatusArea.Nest[1], Status_TextColor, "%s", Player->Name);
		DrawLine(StatusArea.Nest[0] - StatusArea.Padding[0], StatusArea.Nest[1] + 28, StatusArea.Nest[2] + StatusArea.Padding[2], StatusArea.Nest[1] + 28, StatusArea.BorderColor, 2);
		DrawFormatString(StatusArea.Nest[0], StatusArea.Nest[1]+34, Status_TextColor, "ＨＰ %5d\nＭＰ %5d\nＬｖ %5d", Player->Hp, Player->Mp, Player->Level);
		...
		if (Scene == 1) {
			...
			//●次の大問が存在しない（クリアーメッセージの表示）
				...
				case 6: Finish++;//次のメッセージの準備またはTimeAttack関数の終了
				}
			}
		}
		//〔モンスターの攻撃〕
			...
			static int Count = 0;
			static int ATK, DEF, Damage;
			Count++;
			if (Count == 1) {
				//（パッドのリセット）
				PadManager.Active = 0;//パッドの非アクティブ化（判定が終わるまでパッドが使えないように）
				MonsterImgFlag = 0;//モンスター画像の表示
				PlaySoundMem(DamageSound1, DX_PLAYTYPE_BACK, TRUE);

				strcpy(Msg_Battle, "<p>");
				strcat(Msg_Battle, Monster[MonsterNum[MondaiNum]].Name);
				strcat(Msg_Battle, "の　こうげき！</p>");
				Reparse(&MsgBox_Battle);
			...
			if (Count < FPS * 3 / 10) {
				...
				//（モンスターの描画）
				int MonsterX1 = Entire.Nest[0] + 153 + (MsgBox[0].Width - MonsterImgWidth[MonsterNum[MondaiNum]] * 2) / 2;
				int MonsterY1 = Entire.Nest[1] + MonsterBaseLine3 - MonsterImgHeight[MonsterNum[MondaiNum]] * 2;
				int MonsterX2 = MonsterX1 + MonsterImgWidth[MonsterNum[MondaiNum]] * 2;
				int MonsterY2 = MonsterY1 + MonsterImgHeight[MonsterNum[MondaiNum]] * 2;
				DrawExtendGraph(MonsterX1, MonsterY1, MonsterX2, MonsterY2, Monster[MonsterNum[MondaiNum]].Img, true);
			...
			//（モンスターの接近）
				...
				//（ジョイパッドの振動）
				if (Count == FPS / 3) {
					PlaySoundMem(DamageSound2, DX_PLAYTYPE_BACK, TRUE);
					StartJoypadVibration(DX_INPUT_PAD1, 1000, 250);
					//（戦闘計算）
					ATK = Monster[MonsterNum[MondaiNum]].Column[4];
					DEF = Player->Defense;
					Damage = (ATK - DEF) * (100 + rand() % 50) / 100;
					strcat(Msg_Battle, "<p>");
					strcat(Msg_Battle, Player->Name);
					strcat(Msg_Battle, "は　<math>");
					_itoa(Damage, TextBuff, 10);
					strcat(Msg_Battle, TextBuff);
					strcat(Msg_Battle, "</math>の　ダメージを　うけた！</p>");
					Reparse(&MsgBox_Battle);
				}
				if (FPS / 3 <= Count && Count < FPS / 2) {
					//（モンスターの描画）
					int MonsterX1 = Entire.Nest[0] + 53 + (MsgBox[0].Width - MonsterImgWidth[MonsterNum[MondaiNum]] * 4) / 2;
					int MonsterY1 = Entire.Nest[1] + MonsterBaseLine4 - MonsterImgHeight[MonsterNum[MondaiNum]] * 4;
					int MonsterX2 = MonsterX1 + MonsterImgWidth[MonsterNum[MondaiNum]] * 4;
					int MonsterY2 = MonsterY1 + MonsterImgHeight[MonsterNum[MondaiNum]] * 4;
					DrawExtendGraph(MonsterX1, MonsterY1, MonsterX2, MonsterY2, Monster[MonsterNum[MondaiNum]].Img, true);
				}
				ShowMsgBox(Msg_Battle, &MsgBox_Battle);
				//モンスターの攻撃の瞬間
				if (Count == FPS / 2) {
					...
					if (Player->Hp > 0) {
						Count = 0;
						Scene = 0;//モンスターの攻撃から抜ける
						MondaiFlag = 1;//問題を表示
						GaugeFlag = 1;//ゲージの起動
						//（パッドのリセット）
						PadManager.Active = 1;//パッドの非アクティブ化（判定が終わるまでパッドが使えないように）
					}
					//（敗戦シーンへ進む）
					else Scene = 3;
				}
		...
		//〔敗戦〕
			...
			if (Count == 1) {
				//（敗戦メッセージの作成）
				strcpy(Msg_Judge, "<p> You lost. </p>");//時間切れメッセージを取得する
				Reparse(&MsgBox_Judge);//タグの再読み込み指示
				//（敗戦メッセージの色の変更）
			...
			//●敗戦メッセージの表示
				...
				MsgBox_Judge.Location[1] = MonsterBaseLine2 - FPS - Scroll;//敗戦メッセージの表示位置の指定
				Scroll += 2;//敗戦メッセージのスクロール量の更新
			}
			//（終了）
			ShowMsgBox(Msg_Judge, &MsgBox_Judge);
			if (Count >= FPS * 2) {
				...
				if (Finish == 1) {
					...
					Finish = 0;//初期値に戻す
					return -1;//TimeAttack関数の終了（-1を戻り値とする）
				}
				//敗戦メッセージの表示
				switch (ShowMsgBox(Msg_End, &MsgBox_End)) {
				case 6: Finish++;//関数の終了
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	int AreaKosuu;
	LoadAreaSet(".\\File\\StatusArea.csv", &StatusArea, &AreaKosuu, 1, NULL);
	StatusArea.Height += 56;
	StatusArea.Location[0] = (WindowWidth - StatusArea.Width) / 2;
	StatusArea.Location[1] = (WindowHeight - StatusArea.Height) / 2;
	//［プレイヤー］
	struct PLAYER Player = { 0 };
	strcpy(Player.Name, "ゆうしゃ");
	Player.Hp = 100;
	Player.Mp = 10;
	Player.Level = 5;//（0～：レベル1，100～：レベル2，200～：レベル3，300～：レベル4，400～：レベル5，500～：レベル6　・・・）
	Player.Exp = 499;
	Player.Coin = 50;
	Player.Defense = 45;
	...
	while (!ScreenFlip() && !ProcessMessage() && !ClearDrawScreen() && !MathgpUpdateJoypad()) {
		...
		static int NameWidth = GetDrawStringWidth(Player.Name, strlen(Player.Name));
		static int NameX = (StatusArea.Nest[2] - StatusArea.Nest[0] - NameWidth) / 2;
		DrawFormatString(StatusArea.Nest[0] + NameX, StatusArea.Nest[1], TextColor, "%s", Player.Name);
		DrawLine(StatusArea.Nest[0] - StatusArea.Padding[0], StatusArea.Nest[1] + 28, StatusArea.Nest[2] + StatusArea.Padding[2], StatusArea.Nest[1] + 28, StatusArea.BorderColor, 2);
		DrawFormatString(StatusArea.Nest[0], StatusArea.Nest[1] + 34, TextColor, "ＨＰ %5d\nＭＰ %5d\nレベル %3d\n経験値 %3d\nコイン %3d", Player.Hp, Player.Mp, Player.Level, Player.Exp, Player.Coin);
		//●確定ボタンで抜ける
